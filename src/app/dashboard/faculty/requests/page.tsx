...
// //             disabled={!selectedEquipment || !reason.trim()}
// //           >
// //             Submit Request
// //           </Button>
// //         </div>
...
//       // Assumes API returns the created request in data.request
...
export default function FacultyRequestsPage() {
  ...
  useEffect(() => {
    ...
    // Fetch user's equipment requests from /api/e-req (returns an array).
    fetch("/api/e-req", {
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  const handleRequest = async () => {
    ...
    try {
      const res = await fetch("/api/e-req", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      // Append the newly created request to the array.
      setRequests([...requests, data.request]);
      setReason("");
      setSelectedEquipment("");
      setError(null);
