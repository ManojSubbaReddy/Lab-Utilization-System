...
//       // Since the user interface has name inside profile, we use userData.profile.name
...
//           startTime,
//           endTime,
//           userName, // <-- Pass the user name here
//         }),
//       });
//       const data = await res.json();
//       if (!res.ok) {
...
export default function ScheduleLabPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (userString) {
      const userData = JSON.parse(userString);
      // Use either userData.userId or userData._id based on your user object
    ...
    try {
      const res = await fetch("/api/scheduled-labs", {
        ...
        body: JSON.stringify({
          ...
          startTime,
          endTime,
          UserId, // <-- Passing the user's ID here instead of the name
        }),
      });
      const data = await res.json();
      if (!res.ok) {
